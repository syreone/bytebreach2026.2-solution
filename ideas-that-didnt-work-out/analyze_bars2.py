...
is_fg = brightness > 90  # background/black threshold

# for each column, find the topmost foreground pixel and its color
col_top = []
col_color = []
for x in range(w):
    ys = np.where(is_fg[:, x])[0]
    if len(ys) > 0 and (h - ys.min()) > 15:  # ignore tiny noise, need real height
        top_y = ys.min()
        sample_y = min(top_y + 8, h - 1)
        col_top.append(h - top_y)  # height in px
        col_color.append(tuple(int(c) for c in arr[sample_y, x]))
    else:
        col_top.append(0)
        col_color.append((0, 0, 0))

# quantize colors to nearest "bucket" to group similar shades together
def quantize(c, step=40):
    return tuple((v // step) * step for v in c)

qcolors = [quantize(c) for c in col_color]

# run-length encode by quantized color, skipping black/background runs
bars = []
i = 0
while i < w:
    if qcolors[i] == (0, 0, 0):
